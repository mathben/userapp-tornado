...
def config(app_id, cookie_name='ua_session_token'):
    ...
    def func(handler_class):
        def wrap_execute(handler_execute):
            def execute(*args):
                inst_cls = args[0]

                inst_cls.request.userapp = Context(userapp.API(app_id=app_id), cookie_name)

                return handler_execute(*args)
            return execute

        # surcharge _execute fct and recall it after
        ...
        def wrapper(*args):
            ...
            try:
                ...
                if len(permission_result.missing_permissions) > 0:
                    result = {'error_code': 'USER_NOT_AUTHORIZED',
